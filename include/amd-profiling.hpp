...
int64_t tempEdge, tempJunction, tempMemory, tempHbm0, tempHbm1, tempHbm2, tempHbm3;
...
std::string hwCounters[15] = {
        "SQ_INSTS_VALU_ADD_F32","SQ_INSTS_VALU_MUL_F32","SQ_INSTS_VALU_FMA_F32",
        "SQ_INSTS_VALU_TRANS_F32","SQ_INSTS_VALU_MFMA_MOPS_BF16","SQ_INSTS_VALU_MFMA_MOPS_F32",
...
void header(std::ofstream &output){
    ...
    std::string header = 
        "avg_power,energy_acc,curr_power,"
...
void writeData(std::ofstream &output){
    ...
    output << power/1000000.0 << "," << ePower << "," << currPower << ","; 
    ...
    output << tempVrgfx << "," << tempVrmem << "," << tempVrsoc << ",";
    output<< throttleStatus << "," << currUclk << ",";
...
void getData(){
    ...
    pool.enqueue(rocprofiler_device_profiling_session_poll, dp_session_id, &profiling_data[0]);
    ...
    rsmi_dev_metrics_curr_socket_power_get( device, &currPower );
    ...
    rsmi_dev_temp_metric_get( device, RSMI_TEMP_TYPE_EDGE, RSMI_TEMP_CURRENT, &tempEdge );
    ...
    rsmi_dev_metrics_temp_vrgfx_get( device, &tempVrgfx );
    rsmi_dev_metrics_temp_vrmem_get( device, &tempVrmem );
...
void updateHist(){
    ...
    std::copy(currXgmiRead, currXgmiRead+sizeof(currXgmiRead)/sizeof(currXgmiRead[0]), prevXgmiRead);
    std::copy(currXgmiWrite, currXgmiWrite+sizeof(currXgmiWrite)/sizeof(currXgmiWrite[0]), prevXgmiWrite);
